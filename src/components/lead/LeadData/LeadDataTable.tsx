...
type LeadItemGql = {
  id?: string | null;
  _id?: string | { $oid?: string | null } | null;
...
/* ------------------------------- Table ------------------------------- */
const EMPTY_ITEMS: ReadonlyArray<LeadItemGql> = Object.freeze([]); // stable fallback to avoid linter warning
...
export default function LeadDataTable() {
  ...
  const variables = useMemo<LeadsQueryVars>(
    ...
    [page, debouncedSearch, isPending]
  );

  // always fetch on args change
  ...
  // ----------------------------- Normalize ------------------------------
  const pageData = data?.leads ?? previousData?.leads; // keep last page while loading new
  const items = (pageData?.items ?? EMPTY_ITEMS) as LeadItemGql[]; // stable fallback (no new [] each render)
  ...
  const rows: Row[] = useMemo(
    () =>
      items.map((l): Row => ({
        id: l.id ?? (typeof l._id === "string" ? l._id : l._id?.$oid) ?? "",
  ...
  // ---------------------------- Download XLSX ---------------------------
  // Exports selected rows if any; otherwise exports current page rows
  ...
  const showAdvancedCols = rows.some((l) => !!l.leadCode || !!l.assignedRm);
      ...
      <div className="flex flex-col gap-3 border-b border-gray-100 p-4 dark:border-white/[0.05] md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-base font-medium text-gray-800 dark:text-white/90">
            Leads ({mode === "pending" ? "Pending Only" : "All"})
          </h2>
        </div>

        {/* same toolbar, just adds Download button at the end */}
